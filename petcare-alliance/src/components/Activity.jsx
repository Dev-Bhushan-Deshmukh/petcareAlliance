...
<div className='flex gap-3'>
  ...
  <label htmlFor=""className='w-[20%]'>Till</label><input className={`border rounded-md text-slate-500 text-center `} disabled={timetable.activities.daily} type="date" id="appt" name="expiry"  onChange={(e)=>setTimeTable(prev=>({...prev,activities:{...prev.activities,[e.target.name]:e.target.value}}))}/>

<input type="checkbox" name="daily" id="" value={true}  onChange={(e)=>setTimeTable(prev=>({...prev,activities:{...prev.activities,[e.target.name]:e.target.checked ?e.target.name:null}}))} />
<label>Daily</label>
</div>
...
{activityList.map((item)=><div className='bg-white w-[92%] h-[25%] border-l-blue-600 border-l-[5px] rounded-sm shadow-md  mb-2 pl-1 pr-1 flex flex-col '>
...
<div className=' flex-1 flex justify-between items-center'><label htmlFor="" className='font-regular text-slate-400 text-sm'>{`${timetable.activities.start}${timetable.activities.start.split(':')[0]>11 ?'PM':'AM'}- ${timetable.activities.end}${timetable.activities.end.split(':')[0]>11 ?'PM':'AM'}` }</label>
